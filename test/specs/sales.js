...
describe('Sales page', () => {
  it.only('Should Match Sales Screen with Figma Design', async () => {
    ...
    const meta = await sharp(actualPath).metadata()

    // Crop actual image to remove status bar (e.g., 80px from top)

    const croppedBuffer = await sharp(actualPath)
      .extract({
        left: 0,
        top: 45, //  remove top px (status bar)
        width: meta.width,
        height: meta.height - 45,
      })
      .toBuffer()

    // Save cropped image back to actualPath
    fs.writeFileSync(actualPath, croppedBuffer)
    console.log("Cropped image saved at:", actualPath)
    ...
    console.log(
      `Mismatch: ${mismatchPixels} pixels (${mismatchPercentage.toFixed(2)}%)`
    )
